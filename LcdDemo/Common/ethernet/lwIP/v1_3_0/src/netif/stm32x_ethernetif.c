...
#define netifBUFFER_WAIT_DELAY					( 3 / portTICK_RATE_MS )
/* ...and don't look more than this many times. */
#define netifBUFFER_WAIT_ATTEMPTS				( 30 )
...
/* The lwip_buffer is not a fixed array, but instead gets pointed to the buffers
 allocated within this file. */
static unsigned char * s_lwip_buf;
...
static err_t low_level_output(struct netif *netif, struct pbuf *p) {
	...
	for (q = p; q != NULL; q = q->next) {
		...
		vTaskSuspendAll();
		memcpy(&s_lwip_buf[l], (u8_t*) q->payload, q->len);
		xTaskResumeAll();
		l += q->len;
	}
	...
	if (len) {
		...
		if (p != NULL) {
			...
			for (q = p; q != NULL; q = q->next) {
				 ...
				 * variable. */
				vTaskSuspendAll();
				memcpy((u8_t*) q->payload, &s_lwip_buf[l], q->len);
				xTaskResumeAll();
				l = l + q->len;
			}
...
portBASE_TYPE xEthInitialise(void) {
	...
	const unsigned char
			ucMACAddress[] = { configMAC_ADDR0, configMAC_ADDR1, configMAC_ADDR2,
	...
	if (xReturn != pdFAIL) {
		...
		xNVICInit.NVIC_IRQChannel = ENET_IRQ_NUM;
...
unsigned char vSendMACData(unsigned short usDataLen) {
	...
	xTxDescriptor.Buffer1Addr = (unsigned long) s_lwip_buf;
	...
	/* s_lwip_buf is being sent by the Tx descriptor.  Allocate a new buffer. */
	s_lwip_buf = prvGetNextBuffer();
...
unsigned short usGetMACRxData(void) {
	...
	if ((xRxDescriptors[s_ulNextDescriptor].Status & ETH_DMARxDesc_ES) != 0) {
		...
		if (s_ulNextDescriptor >= NUM_RX_DESCRIPTORS) {
			...
			== 0) {
		/* Mark the current buffer as free as s_lwip_buf is going to be set to
		 the buffer that contains the received data. */
		vReturnBuffer(s_lwip_buf);
		...
		s_lwip_buf = (unsigned char *) (xRxDescriptors[s_ulNextDescriptor].Buffer1Addr);
