...
#ifdef STACK_LWIP_130
#define mainNET_TASK_BASE_PRIORITY ( tskIDLE_PRIORITY + 2 )
#endif
...
typedef struct
{
	...
	LcdTextSettings_TypeDef lcdSettings;	/*<< The row on which the message should be displayed. */
} xLCDMessage;
/*-----------------------------------------------------------*/
...
static void prvSetupHardware( void )
{
	...
	/* GPIO0 clock source enable, used by the LCD LCD_CS */
	SCU_APBPeriphClockConfig(__GPIO0, ENABLE);
	GPIO_DeInit(GPIO0);

	/* GPIO3 clock source enable, used by the LCD SSP1_CLK/SSP1_MOSI */
	SCU_APBPeriphClockConfig(__GPIO3, ENABLE);
	GPIO_DeInit(GPIO3);
	
	/* GPIO3 clock source enable, used by the LCD. 
...
LcdTextSettings_TypeDef textSettings;
	...
	textSettings.Position.RowPosition=67;
	textSettings.Position.ColumnPosition=1;
	textSettings.Colors.BackColor=LCD_COLOR_RED;
	textSettings.Colors.ForeColor=LCD_COLOR_WHITE;
	...
	for( ;; )
	{
		...
		//xMessageToSend.lcdSettings.Position.RowPosition=66;
		//xMessageToSend.lcdSettings.Position.ColumnPosition=1;
		//xMessageToSend.lcdSettings.Colors.BackColor=LCD_COLOR_RED;
		//xMessageToSend.lcdSettings.Colors.ForeColor=LCD_COLOR_WHITE;
	    xMessageToSend.lcdSettings = textSettings;
		...
		textSettings.Colors.ForeColor=~textSettings.Colors.ForeColor;
		textSettings.Colors.BackColor=~textSettings.Colors.BackColor;
...
LcdTextSettings_TypeDef textSetting;
	...
	textSetting.Position.ColumnPosition=0;
	textSetting.Position.RowPosition=2;
	textSetting.Colors.BackColor=LCD_COLOR_WHITE;
	textSetting.Colors.ForeColor=LCD_COLOR_BLACK;
	...
	for( ;; )
	{
		...
		textSetting.Position.RowPosition = (xIndex * 11) + 2;
		xMessageToSend.lcdSettings = textSetting;
	...
	for( ;; )
	{
		...
		if( xQueueReceive( *pxLCDQueue, &xReceivedMessage, portMAX_DELAY ) )
		{
			...
			pcString = *xReceivedMessage.ppcMessageToDisplay;
  			LCD_DisplayString(pcString,xReceivedMessage.lcdSettings);
...
/*!
 * Needed by printf implementation. Is is the implementation of puutchar function.
